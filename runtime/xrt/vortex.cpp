...
public:
  ...
  uintptr_t dev_addr_end = dev_addr_start + size;
    ...
    for (uint64_t end = dev_addr + asize; dev_addr < end;
      ...
      printf("DEBUG: upload 0x%lx ptr=%p *ptr=%02x\n", dev_addr, ptr, *(ptr + bo_offset));
  ...
  int download(void *dest, uint64_t dev_addr, uint64_t size) {
    ...
    #else
      CHECK_ERR(xrtBOSync(xrtBuffer, XCL_BO_SYNC_BO_FROM_DEVICE, CACHE_BLOCK_SIZE, bo_offset), {
        dump_xrt_error(xrtDevice_, err);
        return err;
      });
      CHECK_ERR(xrtBORead(xrtBuffer, host_ptr, CACHE_BLOCK_SIZE, bo_offset), {
        dump_xrt_error(xrtDevice_, err);
        return err;
      });
    #endif
...
private:
  ...
  int get_bank_info(uint64_t addr, uint32_t *pIdx, uint64_t *pOff) {
    ...
    uint64_t offset = block_addr * CACHE_BLOCK_SIZE;
    if (pIdx) {
      *pIdx = index;
    }
    if (pOff) {
